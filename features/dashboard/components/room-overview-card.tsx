      ...
      <div className="flex justify-between items-start mb-4">
        ...
        {!isEditing && (
          <button
            ...
            className="text-primary text-sm font-bold bg-gray-100 px-3 py-1.5 rounded-lg hover:bg-secondary-light transition-colors"
          >
            ตั้งค่า
          </button>
        )}
      ...
      {isEditing ? (
        ...
        <div className="space-y-2 text-[15px] pt-4 border-t border-border mt-2">
          ...
          {room.collectionType === "MONTHLY" && (
            <ManualBillGenerator
              roomId={room.id}
              isAutoBilling={room.autoBillingEnabled}
            />
          )}
        </div>
      )}
