...
export default function MembersTable({ roomId }: MembersTableProps) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-border shadow-sm overflow-hidden mb-4">
      ...
      {members.length === 0 ? (
        <div className="p-8 text-center text-text-secondary">ยังไม่มีสมาชิกในห้องนี้</div>
      ) : (
                ...
                <tr key={member.userId} className="hover:bg-primary/5 transition-colors">
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-medium text-text-primary">{member.user.displayName}</span>
                    </div>
                  </td>
